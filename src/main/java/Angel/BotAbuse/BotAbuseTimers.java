                    ...
                    if (removedID != 0) {
                        try {
                            ...
                            guild.removeRoleFromMember(removedID,
                                    baFeature.botConfig.botAbuseRole).queueAfter(5, TimeUnit.MILLISECONDS);
            ...
            timer2.schedule(new TimerTask() {
                ...
                public void run() {
                    ...
                    baFeature.BACore.currentBotAbusers.forEach(id -> {
                        if (serverMembers.contains(guild.getMemberById(id)) &&
                                !guild.getMemberById(id).getRoles().contains(baFeature.botConfig.botAbuseRole)) {
                            guild.addRoleToMember(guild.getMemberById(id),
                                    baFeature.botConfig.botAbuseRole).queue();
                            embed.setAsInfo(defaultTitle, "[System - Role Scanner] Added Bot Abuse Role to "
                                    + guild.getMemberById(id).getAsMention() +
